    ...
    public void UpdateMenuObject() {
        ...
        if(FindAnyObjectByType<LevelManager>()) { //test if you are in the level
            switch(GlobalGameManager.GetCurrentWeek().musicType) {
                case Week.MusicType.SuperTense: { themeMusic = GlobalGameManager.GetCurrentMenuTheme().superTenseMusic; if(themeMusic != null) {  break; } else { goto case Week.MusicType.Tense; } }
                case Week.MusicType.Tense: { themeMusic = GlobalGameManager.GetCurrentMenuTheme().tenseMusic; if(themeMusic != null) {  break; } else { goto case Week.MusicType.Calm; } }
                case Week.MusicType.Calm: { themeMusic = GlobalGameManager.GetCurrentMenuTheme().calmMusic; break; }
            }
        }
